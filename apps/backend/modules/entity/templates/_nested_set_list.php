            ...
            <div class="nodeinteraction createnode">
              <form action="<?php echo url_for('entity_addChild', $record) ?>" method="POST">
                <select name="type" onchange="this.form.submit();">
                  <option selected="selected" disabled="disabled"><?php echo __('Add new ...') ?></option>
              <?php foreach (EntityTypeTable::getInstance()->findAll() as $entityType) : /* @var $entityType EntityType */ ?>
              <?php if ($entityType->className != 'Home') : ?>
                  <option value="<?php echo $entityType->className ?>">
                <?php echo $entityType->name ?>
                </option>
              <?php endif ?>
              <?php endforeach ?>
                </select>
